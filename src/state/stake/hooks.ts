...
// import { smartcoinArtifact as SMRT_ABI } from '../../constants/abis/index'
...
export interface StakingInfoBase {
  ...
  smrtPriceUsd?: number,
  tvl?: number,
  nosellerClaimableBalance? :number,
  tsNum?: number,
  smrtMeta?:any,
  burnNum?: number,
  yourApr?: number,
...
export function useStakingInfo(version: number, pairToFilterBy?: any | null): DoubleSideStakingInfo[] | null {
  ...
  let t1: any = pairToFilterBy?.tokenAmounts[0].token;
  let t2: any = pairToFilterBy?.tokenAmounts[1].token;
  
  const png = PNG[ChainId.AVALANCHE];

  let chef = useChefContract();
  let wavax = useTokenContract(WAVAX[ChainId.AVALANCHE].address)
  let usdc = useTokenContract(`0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664`)
  let smrt = useTokenContract(TOKEN_ADDR)

  let lpToken = useStakingContract();
  let lp = new Token(ChainId.AVALANCHE, LP_ADDR, 18, 'JLP', 'Joe LP Token');

  const queryArgsAccount = useMemo(() => [account ?? undefined], [account])
  const queryArgsPool = useMemo(() => [POOL_INDEX, account ?? undefined], [account])
  const queryArgChefAddr = useMemo(() => [chef?.address ?? undefined], [chef])
  const queryArgLpAddr = useMemo(() => [lpToken?.address ?? undefined], [lpToken]);

  const resultPendingTokens = useSingleCallResult(chef, 'pendingTokens', queryArgsPool)
  const resultUserInfo = useSingleCallResult(chef, 'userInfo', queryArgsPool)
  const resultLpBalance = useSingleCallResult(lpToken, 'balanceOf', queryArgChefAddr)
  const resultStakedAvax = useSingleCallResult(wavax, 'balanceOf', queryArgLpAddr)
  const resultStakedSmrt = useSingleCallResult(smrt, 'balanceOf', queryArgLpAddr)
  const resultJoePerSec = useSingleCallResult(chef, 'joePerSec')
  const resultLpSupply = useSingleCallResult(lpToken, 'totalSupply')
  const resultGlobalStakedUsdc = useSingleCallResult(usdc, 'balanceOf', [`0xA389f9430876455C36478DeEa9769B7Ca4E3DDB1`])
  const resultGlobalStakedWavax = useSingleCallResult(wavax, 'balanceOf', [`0xA389f9430876455C36478DeEa9769B7Ca4E3DDB1`])

  const claimContract = useClaimContract();
  const claimTwitterContract = useClaimTwitterContract();
  
  // let nosellerClaimableBalance = useMemo(() => {
  //   let result = 0;
  //   claimContract?.claimableValueDiamond().then((res) => {
  //     console.log(`wow back`);
  //     console.log(res);
      
  //     if(res && res._hex) {
  //       result = toNum(JSBI.BigInt(res._hex));
  //     }
  //     return result;
  //   })
  ...
  let sUsdc = toNum(toBi(resultGlobalStakedUsdc), 6)
  let sWavax = toNum(toBi(resultGlobalStakedWavax));
  let stakedSmrt = toNum(toBi(resultStakedSmrt));
  let sWavaxForSmrt = toNum(toBi(resultStakedAvax));
  let totalLpTokens = toNum(toBi(resultLpSupply));
  
  let wavaxPrice = sUsdc / sWavax;
  let smrtPriceInAvax = sWavaxForSmrt / stakedSmrt;

  // wat?
  // @TODO: clean me
  let joePerSecBi:any  = toBi(resultJoePerSec);
  joePerSecBi  = JSBI.divide(joePerSecBi, JSBI.BigInt(2))
  let joePerSec:any = JSBI.multiply(joePerSecBi, JSBI.BigInt(1000))
  joePerSec = JSBI.divide(joePerSec, JSBI.BigInt(10 ** 18))
  joePerSec = JSBI.toNumber(joePerSec) / 1000
  let smrtPerSec = joePerSec;
  let smrtPerSecBi = joePerSecBi;

  let smrtPriceUsd = smrtPriceInAvax * wavaxPrice;
  let tvl = (sWavaxForSmrt * wavaxPrice) + (stakedSmrt * smrtPriceUsd)
  let lpTokenVal = tvl / totalLpTokens

  // let pendingJoeBi  = new BigNumber(resultPendingTokens.result ? resultPendingTokens.result?.pendingJoe._hex: 0)
  let pendingJoe = JSBI.BigInt(resultPendingTokens.result ? resultPendingTokens.result?.pendingJoe?._hex: 0)
  let userStakeAmt = JSBI.BigInt(resultUserInfo.result ? resultUserInfo.result?.amount?._hex: 0)
  let totalStaked = toBi(resultLpBalance)
  
  // TODO: think we can remove
  const stakedAmount = JSBI.multiply(JSBI.BigInt(0), JSBI.BigInt(10 ** 18));
  
  let lpPositionUsd = JSBI.toNumber(JSBI.divide(userStakeAmt, JSBI.BigInt(10 ** 18))) * lpTokenVal
  
  // TODO: clean this too
  let poolShare1 = JSBI.divide(totalStaked, JSBI.BigInt(10 ** 18))
  let poolShare2 = JSBI.divide(userStakeAmt, JSBI.BigInt(10 ** 18))
  let poolShareBi = JSBI.divide(poolShare2, poolShare1)
  let f1 = parseFloat(poolShare1.toString())
  let f2 = parseFloat(poolShare2.toString())
  let poolShare = f2/f1;
  poolShare = (poolShare * 100);

  let rewardRate:any = smrtPerSec * (poolShare / 100)
  let rewardRateBi:any = JSBI.multiply(smrtPerSecBi, poolShareBi)
  let dailyReward = rewardRate * 60 * 60 * 24;
  let usdPerDay = dailyReward * smrtPriceUsd;
  ...
  return [{
    stakingRewardAddress: CHEF_ADDR,
    tokens: [t1, t2],
    smrtMeta,
    earnedAmount: new TokenAmount(png, pendingJoe),
    rewardRate: new TokenAmount(png, rewardRateBi),
    totalRewardRate: new TokenAmount(png, smrtPerSecBi),
    stakedAmount: new TokenAmount(lp, userStakeAmt),
    totalStakedAmount: new TokenAmount(lp, totalStaked),
    totalStakedInWavax: new TokenAmount(png, stakedAmount),
    totalStakedInUsd: new TokenAmount(png, stakedAmount),
    tvl,
    usdPerDay,
    lpPositionUsd,
    totalLpTokens,
    smrtPriceUsd: smrtPriceUsd,
    poolShare,
    multiplier: JSBI.BigInt(1),
    periodFinish: new Date(1000000000),
    isPeriodFinished: false,
    getHypotheticalRewardRate: (stakedAmount, totalStakedAmount, totalRewardRate) => {
      // return 0
      
      // let stakedAmountNum = JSBI.divide(stakedAmount.raw, JSBI.BigInt(10 ** 18))
      // let totalStakedAmountNum = JSBI.toNumber(JSBI.divide(totalStakedAmount.raw, JSBI.BigInt(10 ** 18)))
      // let poolShare = JSBI.divide(stakedAmount.raw, totalStakedAmount.raw);
      // let stakedNum = JSBI.toNumber(JSBI.divide(stakedAmount.raw, JSBI.BigInt(10 ** 18))) 
      // let totalStakedNum = JSBI.toNumber(JSBI.divide(totalStakedAmount.raw, JSBI.BigInt(10 ** 18))) 
      // let poolShareNum = stakedNum / totalStakedNum;
      // let rewardsRateNum = JSBI.toNumber(JSBI.divide(totalRewardRate.raw, JSBI.BigInt(10 ** 18)))
      // let rewardShareNum = (poolShareNum * rewardsRateNum) * 100
      
      // let num = JSBI.divide(JSBI.BigInt(rewardShareNum), )
      // let psNum = JSBI.toNumber(JSBI.divide(poolShare, JSBI.BigInt(10 ** 18))) / 100
      // const rewardRate = JSBI.divide(JSBI.multiply(JSBI.BigInt(rewardShareNum), JSBI.BigInt(10 ** 18)), JSBI.BigInt(100))
      const rewardRate = JSBI.BigInt(0);
      return new TokenAmount(png, rewardRate)
    }
  }]
...
export function useTotalPngEarned(): TokenAmount | undefined {
  ...
  // const allProposals = useMultipleContractSingleData([pngContract.address], pngContract.interface, 'totalSupply', [])
  // const results = useMultipleContractSingleData([`0xCC2f1d827b18321254223dF4e84dE399D9Ff116c`], SMRT_ABI, 'totalSupply');
  let smrt = usePngContract();
  let chef = useChefContract();
  const results2 = useSingleCallResult(smrt, 'totalSupply')
  const accountArg = useMemo(() => [account ?? undefined], [account])
