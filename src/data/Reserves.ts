  ...
  const pairAddresses = useMemo(
    () =>
      tokens.map(([tokenA, tokenB]) => {
        return tokenA && tokenB && !tokenA.equals(tokenB) ? `0xf070843Ba9ed0ab85B0d15f9E8D67A5A8E073254` : undefined
      }),
    [tokens, chainId]
  )
  ...
  return useMemo(() => {
    return results.map((result, i) => {
      ...
      return [
        PairState.EXISTS,
        new Pair(
          new TokenAmount(token0, "100001337"), 
          new TokenAmount(token1, "100000069"), chainId ? chainId : ChainId.AVALANCHE)
      ]
    })
